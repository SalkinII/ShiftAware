...
*[Document continues with Phases 4-10...]*

---

## Remaining Phases Summary

Due to document length, the remaining phases are outlined here. Each follows the same task structure with exact file paths, code, and commands.

### Phase 4: Identity Page (Tasks 4.1-4.5)
- 4.1: Add event selection step after member selection
- 4.2: Fetch and display registered events for member
- 4.3: Create emoji picker integration for profile creation
- 4.4: Wire profile creation to create EventRegistration
- 4.5: Load event attribute definitions in create form

### Phase 5: Admin Setup (Tasks 5.1-5.4)
- 5.1: Template assignment UI with checkboxes
- 5.2: Global vs event-specific template toggle
- 5.3: Attribute definitions (verify existing implementation)
- 5.4: Event settings form improvements

### Phase 6: Lane Calendar (Tasks 6.1-6.12)
- 6.1: Create unified LaneCalendar component structure
- 6.2: Implement TimeRuler component (top and bottom)
- 6.3: Add view mode switching (Day/3-Day/Week)
- 6.4: Add date navigation (prev/next/picker)
- 6.5: Implement horizontal time positioning for shifts
- 6.6: Add 15-minute snap grid
- 6.7: Implement snap-to-existing-shift-end
- 6.8: Wire drag from palette with time calculation
- 6.9: Implement resize handles
- 6.10: Wire click-to-edit with sidebar
- 6.11: Add shift glow/highlight on selection
- 6.12: Add export button with html2canvas

### Phase 7: User Features (Tasks 7.1-7.4)
- 7.1: Add voting buttons to shift cards
- 7.2: Wire voting to preferences API
- 7.3: Create preferences sidebar panel
- 7.4: Wire swap request flow

### Phase 8: Admin Team (Tasks 8.1-8.5)
- 8.1: Member list filtered by event
- 8.2: Add existing member picker
- 8.3: Create new member form with emoji picker
- 8.4: Edit member attributes modal
- 8.5: Remove from event action

### Phase 9: Allocation (Tasks 9.1-9.3)
- 9.1: Wire distribution settings to EventConfig API
- 9.2: Add allocation rules UI
- 9.3: Implement preview algorithm results

### Phase 10: Cleanup (Tasks 10.1-10.5)
- 10.1: Remove /app/export page
- 10.2: Update route references
- 10.3: Wire audit rollback UI
- 10.4: Final navigation fixes
...
- [ ] Can create member and register for event
- [ ] Can select identity and event in user flow
- [ ] Admin event selector persists across pages
- [ ] Templates filter by assigned event
- [ ] Lane calendar shows time rulers top and bottom
- [ ] Can drag template to specific time
- [ ] Shift blocks position by time
- [ ] Resize handles work with 15-min snap
- [ ] Click shift edits in sidebar
- [ ] Export captures current view
- [ ] Voting buttons save to DB
- [ ] Swap requests create and match
- [ ] Team members show by event
- [ ] Allocation settings save to EventConfig
- [ ] Rollback actually reverts changes
- [ ] All tests pass

---

**Plan complete and saved to `docs/plans/2026-02-01-shiftaware-v21-implementation.md`.**
